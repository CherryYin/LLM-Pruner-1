...
def main(args):
    ...
    if args.block_wise:
        ...
        kwargs = {
            ...
            "ch_sparsity_dict": {
                item: args.pruning_ratio // 3 for i in range(args.block_attention_layer_start, args.block_attention_layer_end) for item in [model.model.layers[i].self_attn.q_proj, model.model.layers[i].self_attn.k_proj, model.model.layers[i].self_attn.v_proj]
            },
            "forward_fn": forward_fn,
            "ignored_layers":[],
            "channel_groups": {
                #layer.self_attn.W_pack: 3 for layer in model.model.model.layers
            },
            "consecutive_groups": {
                item: layer.self_attn.head_dim for layer in model.model.layers for item in [layer.self_attn.q_proj, layer.self_attn.k_proj, layer.self_attn.v_proj]
            },
            "customized_pruners": {
                model.model.layers[0].input_layernorm.__class__: llama_pruner.hf_rmsnorm_pruner,
            },
            "root_module_types": None, 
            "root_instances": [item for i in range(
        ...
        for i in range(args.iterative_steps):
            print(f"iterative step {i}")

            if pruner_type in ['taylor']:
                example_prompts = get_examples('open_books', tokenizer, args.num_examples, seq_len = 64).to(args.device)
            ...
            for layer in model.model.layers:
                layer.self_attn.hidden_size = layer.self_attn.q_proj.weight.shape[0] 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--block_attention_layer_start', type=int, help='start layer of block attention layers', default=5)
    parser.add_argument('--block_attention_layer_end', type=int, help='end layer of block attention layers', default=6)
    parser.add_argument('--block_mlp_layer_start', type=int, help='start layer of block mlp layers', default=5)
    parser.add_argument('--block_mlp_layer_end', type=int, help='end layer of block mlp layers', default=6)
    ...
    parser.add_argument('--eval_device', type=str, default="cpu", help='eval device')
